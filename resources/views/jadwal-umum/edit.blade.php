    ...
    <section class="section">
        ...
        <div class="card card-primary">
            <div class="card-header">
                <h4>Perbarui Jadwal Umum</h4>
            </div>
            <div class="card-body">
                {{-- Mengarahkan form action ke route update untuk jadwal umum --}}
                <form action="{{ route('jadwal-umum.update', $generalSchedule->id) }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input name="nama_kegiatan" id="nama_kegiatan" type="text" class="form-control" value="{{ old('nama_kegiatan', $generalSchedule->nama_kegiatan) }}">
                        @error('nama_kegiatan')
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="hari" id="hari" class="form-control">
                                    ...
                                    @foreach ($days as $day)
                                        <option value="{{ $day }}" {{ old('hari', $generalSchedule->hari) == $day ? 'selected' : '' }}>{{ $day }}</option>
                                    @endforeach
                                </select>
                                @error('hari')
                                    <p class="text-danger">{{ $message }}</p>
                            ...
                            <div class="form-group">
                                ...
                                <select name="academic_period_id" id="academic_period_id" class="form-control">
                                    ...
                                    @foreach ($academicPeriods as $period)
                                        <option value="{{ $period->id }}" {{ old('academic_period_id', $generalSchedule->academic_period_id) == $period->id ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <select name="time_slot_id_mulai" id="time_slot_id_mulai" class="form-control">
                                    ...
                                    @foreach ($timeSlots as $timeSlot)
                                        <option value="{{ $timeSlot->id }}" {{ old('time_slot_id_mulai', $generalSchedule->time_slot_id_mulai) == $timeSlot->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="time_slot_id_selesai" id="time_slot_id_selesai" class="form-control">
                                    ...
                                    @foreach ($timeSlots as $timeSlot)
                                        <option value="{{ $timeSlot->id }}" {{ old('time_slot_id_selesai', $generalSchedule->time_slot_id_selesai) == $timeSlot->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="keterangan" id="keterangan" class="form-control" style="height: 100px;">{{ old('keterangan', $generalSchedule->keterangan) }}</textarea>
                        @error('keterangan')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>

                    <button type="submit" class="btn btn-primary">Perbarui Jadwal Umum</button>
                </form>
            </div>
        </div>
    </section>
