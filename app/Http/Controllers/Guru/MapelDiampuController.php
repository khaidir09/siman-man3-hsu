...
class MapelDiampuController extends Controller
{
    ...
    public function index()
    {
        // Ambil semua data "pembelajaran" yang dimiliki oleh guru yang login
        $learnings = Learning::where('user_id', Auth::id())
            ->with(['subject', 'room', 'academicPeriod'])
            ->get();

        return view('guru.mapel-diampu', compact('learnings'));
    }
}
